...
export default function DashboardPage() {
  ...
  const generateMutation = useMutation({
    ...
    onError: (err: any) => {
      const msg = err.response?.data?.detail || "Generation failed. Please verify Ollama is active.";
      addToast(msg, "error");
    }
  });
  ...
  const replyMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        platform,
        original_comment: replyTarget,
        reply_style: replyStyle,
        persona_id: null,
        temperature,
        comment_length: "short"
      };
      const resp = await api.post("/comments/reply", payload);
      return resp.data;
    },
    onSuccess: (data) => {
      setResults((prev) => [data, ...prev]);
      setReplyTarget("");
      addToast("Reply response generated!", "success");
    },
    onError: (err: any) => {
      const msg = err.response?.data?.detail || "Reply generation failed.";
      addToast(msg, "error");
    }
  });
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            <Card className={`border transition-colors duration-300 ${isLight ? "bg-white border-zinc-200/80 shadow-sm" : "bg-zinc-950/40 border-zinc-900 backdrop-blur-md"}`}>
              ...
              <CardContent className="flex flex-col gap-5">
                {/* Generation Objective Toggle Pills */}
                <div className="flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      const isObjSelected = commentType === obj.id;
                      return (
                        <button
                          key={obj.id}
                          onClick={() => {
                            setCommentType(obj.id);
                            setContentText("");
                          }}
                          type="button"
            ...
            <Card className={`border transition-colors duration-300 ${isLight ? "bg-white border-zinc-200/80 shadow-sm" : "bg-zinc-950/40 border-zinc-900 backdrop-blur-md"}`}>
              ...
              <CardContent className="flex flex-col gap-3">
                <textarea
                  ...
                  placeholder="Paste the user comment you want to reply to..."
                  rows={2}
                  className={`w-full border rounded-xl px-4 py-2.5 text-xs focus:outline-none ${
                    isLight
                      ...
                      : "bg-zinc-950/80 border-zinc-900 text-zinc-100 placeholder-zinc-700"
                  }`}
                />
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={!replyTarget.trim() || replyMutation.isPending}
                    className={`px-4 py-2 font-bold text-xs rounded-xl text-white cursor-pointer transition-all ${currentPlatColors.bg} ${currentPlatColors.hover}`}
                  >
                    ...
                    <Card key={res.id || index} className={`border transition-all duration-300 relative overflow-hidden ${
                      ...
                      <CardHeader className={`pb-3 border-b flex flex-row items-center justify-between gap-4 pt-5.5 ${isLight ? "border-zinc-100" : "border-zinc-900/60"}`}>
                        <div>
                          ...
                          <CardDescription className={`text-[10px] uppercase font-semibold mt-0.5 ${isLight ? "text-zinc-450" : "text-zinc-500"}`}>
                      ...
                      <CardContent className="pt-4 flex flex-col gap-5">
                        {/* Generated Text */}
                        <p className={`text-sm font-medium leading-relaxed p-4 border rounded-xl select-all ${
                          isLight
                        ...
                        <div className={`grid grid-cols-4 gap-4 border rounded-xl p-3.5 text-center items-center ${
                          isLight ? "bg-zinc-50/50 border-zinc-150" : "bg-zinc-950/60 border-zinc-900/80"
                        }`}>
                          <div className={`flex flex-col gap-1 border-r ${isLight ? "border-zinc-200" : "border-zinc-900"}`}>
                            ...
                            <span className={`text-sm font-black ${getMetricColor(res.humanization_score)}`}>
                          ...
                          <div className={`flex flex-col gap-1 border-r ${isLight ? "border-zinc-200" : "border-zinc-900"}`}>
                            ...
                            <span className="text-xs font-black text-indigo-650 dark:text-indigo-300">
                              {getQualityLabel(res.quality_rating)}
                            </span>
                          </div>
